...
final class SpeakerNamingEngineTests: XCTestCase {
    ...
    func testAlreadyUsedCandidateIsNotReassigned() {
        // Connor is already assigned (fuzzy match on email vs display name should count as used).
        let result = SpeakerNamingEngine.eliminate(
            allClusters: ["Speaker 1", "Speaker 2"],
            candidates: ["Connor McLeod"],
            assigned: ["Speaker 1": "Connor McLeod"]
        )
        // No remaining candidate → Speaker 2 stays unassigned.
        XCTAssertNil(result["Speaker 2"])
    }
